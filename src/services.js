...
import { message, createDataItemSigner, spawn, monitor, result } from "@permaweb/aoconnect";
...
export function sendMessage({ processId, wallet, tags, data }) {
  ...
  const retry = () => fromPromise(() => new Promise(r => setTimeout(r, 500)))()
    .chain(fromPromise(() => message({ process: processId, signer, tags, data })))

  return fromPromise(() => message({ process: processId, signer, tags, data }))()
...
export function register(wallet, address) {
  const findProcess = ({ wallet, address, name, spawnTags }) => {
    return gql(queryForAOS(name, AOS_MODULE), { owners: [address, ""] })
      ...
      .bichain(
        _ => Rejected({ ok: false }),
        results => results.length > 0 ? Resolved(results.reverse()) : Rejected({ ok: true, wallet: jwk, address, name, spawnTags })
      )
  }
